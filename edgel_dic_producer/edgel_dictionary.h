...
void createEdgelDic2(std::vector<std::string>& const ctr_names, std::map<int, Content_Vec>& edgel_dic, std::vector<int>& edgel_counter, std::vector<cv::Mat>& ctr_base, std::vector< std::vector<int> >& img_map);
...
void makeImgMapFile(std::vector< std::vector<int> >& img_map);
